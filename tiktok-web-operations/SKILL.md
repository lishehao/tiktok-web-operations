...
1. **Install, preflight, and ask:** install or upgrade the complete versioned Skill, validate it, prove Chrome control, exact TikTok login, absence of blocking warnings, required thread tools, exact `gpt-5.6-luna/high` thread creation support, no conflicting active TikTok executor or unrelated active Chrome browser session, local time, and a writable ledger. Keep TikTok read-only, release Chrome, return the exact guided direction/duration prompt from `startup-health-check.md`, and wait one user turn.
...
- Before every executor creation or replacement, inspect active TikTok Threads and the live Chrome browser-session owner. Prove the prior owner returned `STOPPED_AND_RELEASED` with no uncertain submission. An unrelated active Thread using Chrome is also a hard ownership blocker; never interrupt or archive it without user authorization. Archiving alone is not release proof.
...
- Never reuse a Chrome tab ID from a prior turn, prompt, ledger, or memory. Choose a live TikTok tab only from the current turn's `openTabs()` result by current URL/title/account context and pass that exact returned object to `user.claimTab()`. Do not pass an `openTabs()` ID to `tabs.get()`. If `claimTab()` reports another browser session, resolve that session UUID through `read_thread`; an active external owner blocks the run without counting as a Feed failure.
