...
3. Validate both release Skills with `quick_validate.py` when available and equivalent structural checks otherwise. Run `scripts/validate_owner_recovery_contract.py`, `scripts/validate_registry_contract.py`, `scripts/validate_role_stage_contract.py`, `scripts/validate_heartbeat_survival_contract.py`, `scripts/validate_round_size_contract.py`, `scripts/validate_parallel_engagement_contract.py`, and `scripts/validate_blocker_minimization_contract.py` when present to enforce reusable-launcher fresh-only dispatch/no historical owner recovery, canonical assignment, role/stage ownership, durable Heartbeat survival, 25–45 view round boundaries, in-view best-effort Like/Favorite/Repost/Comment attempts without persistence checks, hard-blocker whitelisting, scope minimization, and isolated forward-scenario decisions. During an active older runtime use only isolated staging.
