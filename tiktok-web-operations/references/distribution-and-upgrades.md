...
3. Validate both release Skills with `quick_validate.py` when available and equivalent structural checks otherwise. Run `scripts/validate_novice_handoff_contract.py`, `scripts/validate_main_transition_contract.py`, `scripts/validate_profile_lock_contract.py`, `scripts/validate_owner_recovery_contract.py`, `scripts/validate_registry_contract.py`, `scripts/validate_role_stage_contract.py`, `scripts/validate_role_transition_contract.py`, `scripts/validate_callback_first_phase_timer_contract.py`, `scripts/validate_inter_round_cooldown_contract.py`, `scripts/validate_round_size_contract.py`, `scripts/validate_parallel_engagement_contract.py`, `scripts/validate_comment_priority_contract.py`, `scripts/validate_operating_objectives_contract.py`, and `scripts/validate_blocker_minimization_contract.py` when present. These enforce the concise novice handoff, temporary bootstrap to pinned main-task transition, one initial fresh executor per mission, same-mission executor reuse with bounded stale-owner replacement, exact callback handshake/sequence, coordinator-owned callback-first phase timer, machine-clock cooldown, 25–45 view rounds, in-view best-effort actions, high-weight original short comments, bounded no-copy Web research, hard-blocker routing, and isolated forward scenarios. During an active older runtime use only isolated staging.
