...
3. Validate both release Skills with `quick_validate.py` when available and equivalent structural checks otherwise. Run `scripts/validate_novice_handoff_contract.py`, `scripts/validate_distributor_transition_contract.py`, `scripts/validate_profile_lock_contract.py`, `scripts/validate_owner_recovery_contract.py`, `scripts/validate_registry_contract.py`, `scripts/validate_role_stage_contract.py`, `scripts/validate_one_shot_wake_contract.py`, `scripts/validate_inter_round_cooldown_contract.py`, `scripts/validate_round_size_contract.py`, `scripts/validate_parallel_engagement_contract.py`, `scripts/validate_comment_priority_contract.py`, and `scripts/validate_blocker_minimization_contract.py` when present to enforce the concise three-line novice handoff, temporary bootstrap to pinned reusable distributor transition, confirmed account-image gating before fresh dispatch, no historical owner recovery, canonical assignment, role/stage ownership, run/round-unique self-owned one-shot wakes with no distributor callback, 10–20 minute inter-round cooldown, 25–45 view round boundaries, in-view best-effort actions, high-weight original short comments, bounded no-copy Web meme research, hard-blocker whitelisting, scope minimization, and isolated forward-scenario decisions. During an active older runtime use only isolated staging.
