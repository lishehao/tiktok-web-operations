...
3. Validate both release Skills with `quick_validate.py` when available and equivalent structural checks otherwise. Run `scripts/validate_owner_recovery_contract.py` and `scripts/validate_registry_contract.py` when present to enforce stale-owner, canonical-registry, versioned-envelope, and isolated forward-scenario decisions. During an active older runtime use only isolated staging.
