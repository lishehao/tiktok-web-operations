...
| Network/DNS/proxy/TLS/render/client block | one recovery pass; unresolved -> `ROUND_YIELDED`, later Heartbeat resumes the same round |
| Chrome disconnect/stale tab | reconnect same Chrome or create a new owned tab; empty tab list is not browser disconnect |
...
| Uncertain mutation | freeze exact `action_key`; never retry; continue on new posts |
...
Do not create a user question, global pause, task replacement, new round, or
budget reset for these events. Ordinary recovery does not alter the main
scheduler. One executor turn is bounded, but retry persistence continues across
later scheduler wakes until health, stop, cutoff, or current human repair.
...
read-only recheck; the executor never creates a timer. Ask only once while the
evidence remains unchanged, keep the Heartbeat active, and never retry mutations
under the blocker. Historical events never satisfy this whitelist.
